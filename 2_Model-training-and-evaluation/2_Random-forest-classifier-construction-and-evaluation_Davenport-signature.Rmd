---
title: "Classification of whole blood samples into sepsis response groups by mapping to a reference set"
...
Fetching expression values for the 7 predictor genes in all data sets.
```{r fetch_predictor_genes, message=FALSE, warning=FALSE}
# GAinS
gains_microarray_predictors <- data.frame(t(assay(gains_microarray[SRS_genes$ILMN_gene,])))
colnames(gains_microarray_predictors) <- SRS_genes$gene_id

gains_RNAseq_predictors <- data.frame(t(assay(gains_RNAseq[SRS_genes$gene_id,])))

gains_qpcr_predictors <- data.frame(t(assay(gains_qpcr[SRS_genes$gene_name,])))
colnames(gains_qpcr_predictors) <- SRS_genes$gene_id

# dutch500FG
dutch500FG_predictors <- data.frame(t(assay(dutch500FG[SRS_genes$gene_id,])))

# SHIP-TREND
ship_trend_predictors <- data.frame(t(assay(SHIP_TREND[SRS_genes$ILMN_gene,])))
colnames(ship_trend_predictors) <- SRS_genes$gene_id

# Inouye et al
inouye_predictors <- data.frame(t(assay(inouye[SRS_genes$ILMN_gene,])))
colnames(inouye_predictors) <- SRS_genes$gene_id
```
...
reference_set <- data.frame(
  ...
  row.names = c(paste(rownames(gains_microarray_predictors),"IlluminaHT",sep="_"),
                ...
                paste(rownames(gains_qpcr_predictors),"qPCR",sep="_"),
...
pc_coords_reference_set$Study <- c(rep("GAinS", nrow(gains_microarray_predictors)),
                                  ...
                                  rep("GAinS", nrow(gains_qpcr_predictors)),
...
pc_coords_reference_set$Assay <- c(rep("IlluminaHT", nrow(gains_microarray_predictors)),
                                  ...
                                  rep("qPCR", nrow(gains_qpcr_predictors)),
...
batch <- c(rep(1, nrow(gains_microarray_predictors)),
           ...
           rep(3, nrow(gains_qpcr_predictors)),
           rep(2, nrow(dutch500FG_predictors)),
           rep(1, nrow(ship_trend_predictors)),
           rep(4, nrow(inouye_predictors)))
```

Aligining batches with the mutual nearest neighbours (mNN) algorithm
```{r align_batches, message=FALSE, warning=FALSE}
reference_set_aligned <- data.frame(t(assay(mnnCorrect(t(reference_set), 
                                                       batch = batch, 
                                                       merge.order = list(1,2,3,4),
                                                       k = 50))))
```
...
pc_coords_reference_set$SRS_unsup <- c(
  ...
  gsub("2","SRS2",gsub("1","SRS1",gains_qpcr$SRSUnsup)),
  rep("SRS3", nrow(dutch500FG_predictors)),
  rep("SRS3", nrow(ship_trend_predictors)),
  rep("SRS3", nrow(inouye_predictors)))
pc_coords_reference_set$SRS_model <- c(gains_microarray$SRS_New,
...
ggplot(pc_coords_reference_set, aes(x=-PC1, y=PC2)) +
  geom_point(aes(color=Study, shape=Assay), size=3) +
  theme_classic()
...
training_samples_qpcr <- as.tibble(colData(gains_qpcr)) %>% 
  arrange(GAinSID, Day) %>% 
  filter(!duplicated(GAinSID)) %>% 
  filter(!is.na(SRSUnsup)) %>% 
  transmute(SampleID = paste(SampleID,"qPCR",sep="_"),
            SRS = paste("SRS",SRSUnsup,sep=""))
...
training_set <- data.frame(reference_set_aligned[c(training_samples_microarray$SampleID, 
                                                   ...
                                                   training_samples_qpcr$SampleID, 
...
model_rf <- readRDS("../results/classifier_models/SRS-classifier_7-genes_RF_trained-on-mNN-aligned-reference-set.rds")
```
...
Building a diffusion map to order samples into the SRS continuum.
```{r build_diffusion_map, message=FALSE, warning=FALSE}
set.seed(1)
sepsis_diffusion_map <- DiffusionMap(reference_set_aligned, n_eigs=3)
```

Estimating a diffusion pseudotime variable, with the tip set to be a random sample in the SRS1 group
...
ggplot(dc_coords, aes(x = DC1, y = DC2)) +
  ...
  scale_color_gradient2(low = "lightgrey", mid = "steelblue", high = "darkblue", midpoint = 6) +
  xlab("Diffusion component 1") + 
  ylab("Diffusion component 2") +
  theme_classic()
```

The diffusion map clearly separates samples by SRS group, gradually going from healthy to SRS2 and SRS1. Thus, we will use the difussion pseudotime variable to derive a sepsis stratification score (SRSq). 
...
The resulting risk score looks as follows.
...
The resulting risk scores look as follows when stratified by technology and SRS group.
...
pc_coords_reference_set$Study_assay <- factor(pc_coords_reference_set$Study_assay, 
                                              levels=c("FINRISK (IlluminaHT)","SHIP-TREND (IlluminaHT)","500FG (RNA-seq)",
                                                       "GAinS (IlluminaHT)", "GAinS (qPCR)", "GAinS (RNA-seq)")) 
...
set.seed(1)
model_rf_SRSq <- train(SRSq~., data=training_set[,c(1:7,9)],
                       method="rf", 
                       metric="RMSE", 
                       trControl=ctrl, 
                       tuneGrid=grid_rf)
...
model_rf_SRSq <- readRDS("../results/classifier_models/SRSq-predictor_7-genes_RF_trained-on-mNN-aligned-reference-set.rds")
```
...
Predicting labels in the entire refrence set
...
gains_qpcr$SRS_rf_mnn <- pc_coords_reference_set$SRS_rf[pc_coords_reference_set$Study=="GAinS" & pc_coords_reference_set$Assay == "qPCR"]
...
gains_qpcr$SRSq <- pc_coords_reference_set$SRSq[pc_coords_reference_set$Study=="GAinS" & pc_coords_reference_set$Assay == "qPCR"]
```

When comparing the SRS predictions of the random forest model for testing samples with previous labels, we observe the following degree of agreement:
...
shared_samples_qpcr <- intersect(colnames(gains_RNAseq), colnames(gains_qpcr))
jaccard_indexes_gains_qpcr <- data.frame(
  sapply(c("SRS1","SRS2"), FUN=function(i) {
    sapply(c("SRS1","SRS2"), FUN=function(j){
      getJaccardIndex(colnames(gains_qpcr[,shared_samples_qpcr])[gains_qpcr[,shared_samples_qpcr]$SRS_rf_mnn==i], 
                      colnames(gains_RNAseq[,shared_samples_qpcr])[gains_RNAseq[,shared_samples_qpcr]$SRS_rf_mnn==j])
      })
    }),
  row.names = c("SRS1 (RNA-seq)","SRS2 (RNA-seq)")
  )
colnames(jaccard_indexes_gains_qpcr) <- paste(c("SRS1","SRS2"),"(qPCR)",sep=" ")

jaccard_indexes_gains <- cbind(jaccard_indexes_gains_microarray,jaccard_indexes_gains_qpcr)

pheatmap::pheatmap(t(jaccard_indexes_gains), cellheight=20, cellwidth=30,
                   annotation_row = data.frame(Assay=c(rep("Microarray", 2),
                                                       rep("qPCR",2)),
                                               row.names = colnames(jaccard_indexes_gains)))
```

This can also be visualised in the upset plot below
```{r create_upset_plot, message=FALSE, warning=FALSE, echo=FALSE}
all_samples <- unique(c(colnames(gains_microarray),colnames(gains_RNAseq),colnames(gains_qpcr)))
upset(
  fromList(
    list(
      ...
      "qPCR (SRS1)" = intersect(all_samples, colnames(gains_qpcr)[gains_qpcr$SRS_rf_mnn == "SRS1"]),
      ...
      "qPCR (SRS2)" = intersect(all_samples, colnames(gains_qpcr)[gains_qpcr$SRS_rf_mnn == "SRS2"]),
      
      #"Microarray (SRS3)" = intersect(all_samples, colnames(gains_microarray)[gains_microarray$SRS_rf_mnn == "SRS3"]),
      "RNA-seq (SRS3)" = intersect(all_samples, colnames(gains_RNAseq)[gains_RNAseq$SRS_rf_mnn == "SRS3"]),
      "qPCR (SRS3)" = intersect(all_samples, colnames(gains_qpcr)[gains_qpcr$SRS_rf_mnn == "SRS3"])
      )
    ), 
  order.by = "freq", 
  sets = c("Microarray (SRS1)","Microarray (SRS2)",
           "RNA-seq (SRS1)", "RNA-seq (SRS2)","RNA-seq (SRS3)",
           "qPCR (SRS1)", "qPCR (SRS2)", "qPCR (SRS3)"),
  keep.order = T,
  nsets = 9
)
```
...
write.table(DEGs_microarray[order(-DEGs_microarray$logFC),], 
            file = "../results/differential_gene_expression/GAinS/GAinS-microarray_DEGs_SRS1-vs-SRS2_limma.tsv",
            quote = F, row.names = F, sep="\t")

# RNA-seq
write.table(DEGs_RNAseq[order(-DEGs_RNAseq$logFC),], 
            file = "../results/differential_gene_expression/GAinS/GAinS-RNAseq_DEGs_SRS1-vs-SRS2_limma.tsv",
            quote = F, row.names = F, sep="\t")
...
write.table(DEGs_microarray_SRSq[order(-DEGs_microarray_SRSq$logFC),], 
            file = "../results/differential_gene_expression/GAinS/GAinS-microarray_DEGs_SRSq_limma.tsv",
            quote = F, row.names = F, sep="\t")

# RNA-seq
write.table(DEGs_RNAseq_SRSq[order(-DEGs_RNAseq_SRSq$logFC),], 
            file = "../results/differential_gene_expression/GAinS/GAinS-RNAseq_DEGs_SRSq_limma.tsv",
            quote = F, row.names = F, sep="\t")
```
...
load("../results/differential_gene_expression/GAinS/XGR-results-GAinS.rdata")
```
...
```{r merege_outcome_demographic_and_secondary_infections_data, message=FALSE, warning=FALSE}
...
We make sure to set all the relevant variables as factor and numeric types. Moreover, we create a variable which encodes age as "decade of life". 
...
Testing for the significance of this association using a proportional odds logistic regression
```{r run_POLR_for_SRSq_vs_ICUAinf, message=FALSE, warning=FALSE}
polr_res_microarray <- coef(
  summary(
    polr(ICUAcqInfScore~SRSq_microarray_last,
         data = gains_outdat[!is.na(gains_outdat$SRSq_microarray_last),])
    )
  )
polr_res_microarray <- cbind(polr_res_microarray, 
                              "p value" = pnorm(abs(polr_res_microarray[, "t value"]), lower.tail = FALSE)*2)

polr_res_RNAseq <- coef(
  summary(
    polr(ICUAcqInfScore~SRSq_RNAseq_last,
         data = gains_outdat[!is.na(gains_outdat$SRSq_RNAseq_last),])
    )
  )
polr_res_RNAseq <- cbind(polr_res_RNAseq,
                         "p value" = pnorm(abs(polr_res_RNAseq[, "t value"]), lower.tail = FALSE)*2)
...
cox_univariate_microarray <- data.frame(
  variable = c("SRS (SRS1 vs SRS2)","SRSq (per 0.1 unit increase)","SOFA score (per unit increase)", "APACHE score (per unit increase)"),
...
for(sample_id in gains_cell_counts$SampleID){
  ...
  # qPCR
  if( !sample_id %in% colnames(gains_qpcr) ){
    SRS_qpcr <- NA
    SRSq_qpcr <- NA
  } else{
    dat_qpcr <- colData(gains_qpcr)[sample_id,]
    SRS_qpcr = as.character(dat_qpcr$SRS_rf_mnn)
    SRSq_qpcr = round(dat_qpcr$SRSq,3)
  }
  
  SRS_labels_for_cell_counts <- rbind(SRS_labels_for_cell_counts, 
                                      c(SRS_microarray,SRS_RNAseq, SRSq_microarray, SRSq_RNAseq, SRS_qpcr, SRSq_qpcr))
...
gains_cell_counts$SRSq_qpcr <- as.numeric(gains_cell_counts$SRSq_qpcr)
```
...
## Exporting models and reference sets as R data sets
Exporting classifier models
```{r export_models, eval=FALSE}
saveRDS(model_rf, "../results/classifier_models/SRS-classifier_7-genes_RF_trained-on-mNN-aligned-reference-set-with-affymetrix.rds")
saveRDS(model_rf_SRSq, "../results/classifier_models/SRSq-predictor_7-genes_RF_trained-on-mNN-aligned-reference-set-with-affymetrix.rds")
```

Exporting reference set for mNN alignment
```{r export_reference_set, eval=FALSE}
write.table(reference_set_aligned, "../results/classifier_models/reference-set-for-mNN-alignment.tsv", sep="\t", quote=F)
```

We convert the SRS/SRSq predictions to a format suitable for future reference.
```{r format_predictions_to_export, eval=FALSE}
SRS_predictions_gains <- rbind(
  ...
  data.frame(
    Sample_id=gains_qpcr$SampleID,
    Assay="qPCR",
    SRS=gains_qpcr$SRS_rf_mnn,
    SRSq=round(gains_qpcr$SRSq, 4)
    )
)

head(SRS_predictions_gains)
```
...
write.table(SRS_predictions_gains,
            "../results/classifier_predictions/full-gains-SRS-predictions_mNN-RF.tsv",
